...
export function MatchResultPanel({ loading, result }: Props) {
  ...
  const safeResult = {
    ...
    unmet_or_unknown_deal_breakers: result.unmet_or_unknown_deal_breakers ?? [],
    ...
    <div>
      ...
      {safeResult.unmet_or_unknown_deal_breakers.length > 0 && (
        <p style={{ color: '#b91c1c' }}>
          Deal breakers violated: {safeResult.unmet_or_unknown_deal_breakers.join(', ')}
        </p>
      )}
      ...
      <p>
        Recommendation: <strong>{safeResult.recommendation.message || safeResult.recommendation.label}</strong>
      </p>
    </div>
  );
}
